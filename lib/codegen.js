...
module.exports = function generateCode(hyperdispatch, { esm = false }) {
  ...
  str += 'const ERRORS = {\n'
  str += "  NONEXISTENT_ROUTE: 'NONEXISTENT_ROUTE',\n"
  str += "  HANDLER_NOT_FOUND_BY_ID: 'HANDLER_NOT_FOUND_BY_ID',\n"
  str += "  ROUTE_NOT_FOUND_BY_NAME: 'ROUTE_NOT_FOUND_BY_NAME'\n"
  str += '}\n'
  str += '\n'

  str += 'class DispatchError extends Error {\n'
  str += '  constructor (code, message, fn = DispatchError) {\n'
  str += '    super(message ? `${code}: ${message}` : code)\n'
  str += '    this.code = code\n'
  str += '    this.isDispatchError = true\n'
  str += '    if (Error.captureStackTrace) {\n'
  str += '      Error.captureStackTrace(this, fn)\n'
  str += '    }\n'
  str += '  }\n'
  str += '\n'
  str += '  static isDispatchError (err) {\n'
  str += '    return err?.isDispatchError === true\n'
  str += '  }\n'
  str += '\n'
  str += '  static NONEXISTENT_ROUTE (name) {\n'
  str +=
    '    return new DispatchError(ERRORS.NONEXISTENT_ROUTE, name, DispatchError.NONEXISTENT_ROUTE)\n'
  str += '  }\n'
  str += '\n'
  str += '  static HANDLER_NOT_FOUND_BY_ID (id) {\n'
  str +=
    '    return new DispatchError(ERRORS.HANDLER_NOT_FOUND_BY_ID, id, DispatchError.HANDLER_NOT_FOUND_BY_ID)\n'
  str += '  }\n'
  str += '\n'
  str += '  static ROUTE_NOT_FOUND_BY_NAME (name) {\n'
  str +=
    '    return new DispatchError(ERRORS.ROUTE_NOT_FOUND_BY_NAME, name, DispatchError.ROUTE_NOT_FOUND_BY_NAME)\n'
  str += '  }\n'
  str += '}\n'
  str += '\n'
